function formatMath(text) {
    ...
    // Xử lý tất cả các biểu thức thay thế
    ...
    text = text.replace(/\\(sin|cos|tan)\(([^)]+)\)/g, '$1($2)');

    // Xử lý số mũ (lũy thừa): a^b -> a^(b)
    text = text.replace(/\^(\{([^}]+)\}|[a-zA-Z0-9]+)/g, (_, exp) => `^(${exp.replace(/[{}]/g, '')})`);

    // Xử lý chỉ số dưới (subscript): a_b -> a_(b)
    text = text.replace(/_(\{([^}]+)\}|[a-zA-Z0-9]+)/g, (_, sub) => `_(${sub.replace(/[{}]/g, '')})`);

    // Giữ nguyên \text{...} thay vì xóa nó
