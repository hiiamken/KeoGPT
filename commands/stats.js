...
async function handleStatsCommand(interaction, isSlash = true) {
  ...
  userData.total_threads = threadCountRows[0].count;
        ...
        const [rankingRows] = await db.pool.execute(
            ...
            `, // Order by monthly_points, not total_points
            [startOfMonth]
        );
        ...
        const embed = new EmbedBuilder()
            ...
            .setThumbnail(
              interaction.user?.displayAvatarURL() || interaction.author?.displayAvatarURL() // Use user/author consistently
            )
            .addFields(
                {
                    name: "📊 Tổng quan",
        ...
        return { embeds: [embed], ephemeral: !isSlash }; // Return the embed data
    } catch (error) {
...
module.exports = {
    ...
    async execute(interaction) {
        // No need to defer here if we're returning the embed data directly.
        const embedData = await handleStatsCommand(interaction); // Await the result
